...
export function getDelta({ctrlKey, deltaX, deltaY, deltaMode}: SubWheelEvent, pixelunit: number = 32): WheelDelta{
    ...
    if (deltaMode >= 1){
        ...
        return {
            ...
            deltaX < 0 ?
            unit*Math.min(-1, Math.ceil(deltaX/pixelunit)) :
            0,
            y: deltaY > 0 ?
            unit*Math.max(1, Math.floor(deltaY/pixelunit)) :
            deltaY < 0 ?
            unit*Math.min(-1, Math.ceil(deltaY/pixelunit)) :
            0,
        };
    }
}
